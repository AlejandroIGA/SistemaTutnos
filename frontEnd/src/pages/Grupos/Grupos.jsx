...
const Grupos = () => {
  ...
  const manejarBusqueda = async () => {
    try {
      ...
      notification.error({ message: "No se encontraron resultados" });
      setDatos([]);
    } finally {
      setCargando(false);
    }
  ...
  const manejarAgregarGrupo = async (valores) => {
    try {
      if (grupoEnEdicion) {
        // Si el grupo ya estaba inactivo y el nuevo valor también es inactivo, bloquear
        if (
          (grupoEnEdicion.estado === false ||
            grupoEnEdicion.estado === 0 ||
            (typeof grupoEnEdicion.estado === "string" &&
              grupoEnEdicion.estado.toLowerCase() === "inactivo")) &&
          valores.estado === false
        ) {
          notification.warning({
            message:
              "El grupo ya está inactivo, no puedes seleccionarlo de nuevo.",
          });
          return;
        }

        // Normalizar valor de estado
        if (valores.estado === undefined) {
          valores.estado =
            grupoEnEdicion.estado === true ||
            grupoEnEdicion.estado === 1 ||
            (typeof grupoEnEdicion.estado === "string" &&
              grupoEnEdicion.estado.toLowerCase() === "activo");
        } else {
